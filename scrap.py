    ...
    try :
        ...
        try:
            ...
            popup = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.XPATH, "//div[contains(@class, 'cc-window')]//a[contains(@class, 'cc-btn')]")) # This XPath is an example; adjust based on the actual popup
            )
            popup.click()
        ...
        dropdown = driver.find_element(By.XPATH, "//div[contains(text(), 'Current Season')]")
        ...
        WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.XPATH, "//div[contains(text(), 'All-time')]"))).click()
        ...
        while True:
            ...
            while (not found_10): # Sometimes it didn't load well
                if (consecutive_errors > 4):
                    print("WARNING: Too many consecutive errors, this is only OK if it's the last page")
                    
                if not first_loop:
                    time.sleep(0.01)
                first_loop = False
                players = soup.find_all('div', class_='bga-ranking-entry')  # Container for each player
                if len(players) == 10:
                    # Check if the first player is the same as the previous iteration
                    first_player = players[0].find('a', class_='playername').text.strip()
        ...
        sorted_players = sorted(unique_players.items(), key=lambda item: item[1], reverse=True)
