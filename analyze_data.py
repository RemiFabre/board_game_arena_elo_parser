...
def plot_smooth(df, name):
    ...
    for elo_start in elo_starts:
        ...
        sns.kdeplot(filtered_df['ELO'], color='blue', shade=True, label='ELO Distribution', bw_adjust=0.5)
        ...
        percentiles = {f'Top {round(100-p, 1)}%': round(np.percentile(filtered_df['ELO'], p), 0) for p in percentiles_values}
        ...
        plt.ylabel('Density')
        if elo_start == 0:
            plt.title(f'Smooth ELO Distribution of ALL {name} Players ({len(filtered_df)} players)')
        else :
            plt.title(f'Smooth ELO Distribution of {name} Players above {elo_start} ELO ({len(filtered_df)} players)')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        if elo_start == 0:
